...
def evaluate(loader, model):
    ...
    return torch.tensor(war, device=device, dtype=torch.float), \
        (torch.cat(all_labels_lst).cpu().numpy(), torch.cat(all_predictions_lst).cpu().numpy())


def train(loader, model, loss_criterion, optimizer):
    model.train()
    losses = torch.zeros(len(loader)).to(device)
    for batch_idx, (inputs, labels, descriptions) in enumerate(loader):
        with autocast():
            optimizer.zero_grad()
...
if __name__ == "__main__":
    ...
    scaler = GradScaler()
    scheduler = lr_scheduler.MultiStepLR(optimizer, milestones=[15, 35, 50], gamma=0.1)
    # scheduler = lr_scheduler.CosineAnnealingLR(optimizer, T_max=cnf.num_epochs//2)
    ...
    for e in range(cnf.num_epochs):
        ...
        if cnf.is_master and train_loss < last_loss:  # lowest loss
            last_loss = train_loss
            model_filename = os.path.join(MODELS_FOLDER, 'fold_{}_epoch_{}.pth'.format(cnf.fold, e))
        ...
        if war > w:
            ...
            if cnf.is_master:
                ...
                wandb.log({"conf_mat": wandb.plot.confusion_matrix(preds=pd.reshape(-1), y_true=gt.reshape(-1), class_names=CLASSES)})
        scheduler.step()
